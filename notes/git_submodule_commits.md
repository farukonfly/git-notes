...
### ⚠️ 关于 `git push` 的关键警告与 `-dirty` 状态

**疑问：在主项目运行 `git push` 会自动把对子模块的修改也推送到子模块的仓库吗？**
👉 **答：默认绝对不会！** 
主项目的 `git push` 只会推送主项目自己的文件（包括那个更新后的指针），**它不管子模块死活**。如果你更新了指针并推了主项目，但忘记推子模块的代码，你的队友拉取代码时就会遭遇“缺文件（找不到对应 Commit ID）”的灾难。

**什么是 `-dirty` 尾缀？**
如果你在主项目中看到类似这样的 diff：
`-Subproject commit 9c07e...`
`+Subproject commit 9c07e...-dirty`
**`-dirty`（脏的）** 的意思是：你此时此刻在子模块这个文件夹里，**不但没推送，甚至连本地 `commit` 都还没有做！**里面有处于修改状态的、没保存到本地 Git 数据库的“野文件”。Git 无法为一个“浑身是泥”的目录颁发正式的指针号。
